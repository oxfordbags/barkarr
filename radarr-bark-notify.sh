...
# =============================================================================
# Radarr to Bark iOS Notification Script
# =============================================================================
# This script sends Radarr event notifications to a Bark iOS server.
...
# 2. Make this script executable: chmod +x radarr-bark-notify.sh
# 3. In Radarr: Settings > Connect > Add > Custom Script
...
Indexer: ${radarr_release_indexer}"
        ...
        if [ -n "$RADARR_URL" ] && [ -n "$radarr_movie_id" ]; then
            url="${RADARR_URL}/movie/${radarr_movie_id}"
        else
            url="$RADARR_URL"
        fi
...
New Quality: ${radarr_moviefile_quality}"
    ...
    Health)
        ...
        case "$radarr_health_issue_level" in
            Error)
                emoji="🔴"
                ;;
            Warning)
        ...
        title="$emoji Radarr Health: $radarr_health_issue_level"
        body="Type: $radarr_health_issue_type
Message: $radarr_health_issue_message
Wiki: $radarr_health_issue_wiki"

        # Build URL - prefer Radarr system status page over wiki
        if [ -n "$RADARR_URL" ]; then
            url="${RADARR_URL}/system/status"
        else
    ...
    Test)
        ...
        body="This is a test notification from Radarr to Bark. If you're seeing this, the connection is working! 🎉"

        # Escape for JSON
        title=$(escape_json "$title")
        body=$(escape_json "$body")
        url=$(escape_json "$RADARR_URL")

        send_bark_notification "$title" "$body" "$url" ""
        ;;

    *)
        echo "Unknown event type: $radarr_eventtype"
        echo "Available Radarr environment variables:"
        env | grep ^radarr_ | sort
        exit 1
        ;;
esac
